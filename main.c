...
static inline int PrintCmd(char *mesBuf, uint8_t sender_address, uint8_t *buf,
		size_t length) {
	led_command_t cmd;
	int retval = 0;
	...
	case LED_COMMAND_GET_TYPES:
		...
		retval += printLEDTypes(mesBuf+retval,200-retval,buf+1,length-1);
		break;
	case LED_COMMAND_GET_PWM_SETTINGS:
		// unpack PWM settings and put into mesBuf here
		retval += printPWMSettings(mesBuf+retval,200-retval,buf+1,length-1);
		break;
	default:
		return 0;
	}
...
void daisyPacketReceived(uint8_t receive_address, uint8_t sender_address,
	...
	case DAISY_ADDR_MASTER:
		cnt = PrintCmd(mesBuf,sender_address,buf,length);
		break;
	}
